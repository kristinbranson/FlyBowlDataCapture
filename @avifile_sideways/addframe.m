...
%   which is created with AVIFILE_SIDEWAYS.  FRAME can be either an indexed image
...
%      aviobj = close(aviobj);
%
%   See also AVIFILE_SIDEWAYS, AVIFILE_SIDEWAYS/CLOSE, and MOVIE2AVI.
...
if ~isa(aviobj,'avifile_sideways')
  error('MATLAB:addframe:invalidAviFileObjectInput','First input must be an avifile_sideways object.');
end
...
for i = 1:numframes
  ...
  for j = 1:MovieLength
    ...
    if aviobj.MainHeader.TotalFrames==0
      ...
      % switch width & height when storing
      aviobj.MainHeader.Width = height;
      aviobj.MainHeader.Height = width;
      %aviobj.MainHeader.Width = width;
      %aviobj.MainHeader.Height = height;
      aviobj.Bitmapheader.biWidth = height;
      aviobj.Bitmapheader.biHeight = width;
    ...
    if ispc
      ...
      avi('addframe',frame, aviobj.Bitmapheader, ...
...
count = fwrite(fid,chunkdata,'uint8');
...
if height ~= aviobj.MainHeader.Width
  error('MATLAB:addframe:invalidFrameSize','Frame must be %d by %d.', ...
		aviobj.MainHeader.Height,aviobj.MainHeader.Width)
elseif width ~= aviobj.MainHeader.Height
  error('MATLAB:addframe:invalidFrameSize','Frame must be %d by %d.', ...
		aviobj.MainHeader.Height,aviobj.MainHeader.Width)
end
...
if (numcomps == 3)
  X = X(:,:,[3 2 1]); % switch color order to BGR
  X = permute(X, [3 1 2]); 
  ...
  % ...
  % WAS:
  % 1 b(1,1) g(1,1) r(1,1) b(1,2) g(1,2) r(1,2) ... 
  % 2 b(2,1) g(2,1) r(2,1) b(2,2) g(2,2) r(2,2) ... 
  % 3 b(3,1) g(3,1) r(3,1) b(3,2) g(3,2) r(3,2) ... 
  % ...
end
...
if (tmp > 0)
    padding = 4 - tmp;
    X = cat(1, X, repmat(uint8(0), [padding size(X,2)]));
end
