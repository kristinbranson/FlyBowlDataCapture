function aviobj = avifile_sideways(filename,varargin)
%AVIFILE_SIDEWAYS Create a new AVI file
%   AVIOBJ = AVIFILE_SIDEWAYS(FILENAME) creates an AVIFILE object AVIOBJ with the
%   default parameter values.  If FILENAME does not include an extension,
%   then '.avi' will be used.  Use AVIFILE_SIDEWAYS/CLOSE to close the file opened by
%   AVIFILE_SIDEWAYS.  Use "clear mex" to close all open AVI files.
%
%   AVIOBJ = AVIFILE_SIDEWAYS(FILENAME,'PropertyName',VALUE,'PropertyName',VALUE,...)
%   returns an AVIFILE_SIDEWAYS object with the specified property values.
%
%   AVIFILE_SIDEWAYS parameters
%
...
%
%
%   AVIFILE_SIDEWAYS properties may also be set using MATLAB structure syntax.  For
%   example, to set the Quality property to 100 use the following syntax: 
%
%      aviobj = AVIFILE_SIDEWAYS(filename);
%      aviobj.Quality = 100;
%
% 
%   Example: 
...
%      fact = 10*sin(t);
%      fig=figure;
%      aviobj = avifile_sideways('example.avi')
%      [x,y,z] = peaks;
%      for k=1:length(fact)
%          h = surf(x,y,fact(k)*z);
%          axis([-3 3 -3 3 -80 80])
...
%
%
%   See also AVIFILE_SIDEWAYS/ADDFRAME, AVIFILE_SIDEWAYS/CLOSE, MOVIE2AVI.

%   Copyright 1984-2007 The MathWorks, Inc.
%   $Revision: 1.1.6.11 $  $Date: 2008/04/21 16:25:18 $
       
if nargin == 0
  error('MATLAB:avifile_sideways:noFilenameProvided','You must provide a filename as the input.');   
elseif ~isa(filename,'char')
  error('MATLAB:avifile_sideways:invalidFilenameArgument','The first input argument must be a filename.');
end
...
if ispc
  aviobj = class(aviobj,'avifile_sideways');
...
if isunix
  ...
  if fid == -1
    error('MATLAB:avifile_sideways:errorCreatingTempFile',['Error creating temporary file' aviobj.TempDataFile '.  Check directory permissions. ']);
  end
  fclose(fid);
  
  fid = fopen(aviobj.Filename,'a','l');
  if fid == -1
    error('MATLAB:avifile_sideways:errorCreatingAVI','Error creating AVI file. Check directory permissions.');
  end
  fclose(fid);
end
...
% The current default codec for AVIFILE_SIDEWAYS on Windows is Indeo5.  
% Indeo5 does not ship on newer Windows platforms (XP x64, Vista and later)
% For the default case of avifile_sideways
% (i.e. mov = avifile_sideways('myfile.mov'); ), warn the user if Indeo5 is
...
if mmcompinfo('video','video 5.1') == -1
    ...
    warning('MATLAB:avifile_sideways:indeo5NotFound', ...
