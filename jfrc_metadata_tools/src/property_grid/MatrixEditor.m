...
classdef MatrixEditor < UIControl
    ...
    methods
        ...
        function self = Instantiate(self, parent)
            ...
            self.JideTable = com.jidesoft.grid.ContextSensitiveTable(self.JideTableModel);
            self.JideTable.setCellSelectionEnabled(true);
            pixelpos = getpixelposition(panel);
            [control,self.Table] = javacomponent(self.JideTable, [10 10 pixelpos(3)-20 pixelpos(4)-20], panel);
            set(self.Table, ...
                'Units', 'normalized');
    ...
    methods (Static, Access = private)
        ...
        % Fired when a matrix entry has been changed.
            ...
            if ischar(entryvalue)
                javavalue = self.Type.GetPrimitiveJavaValue(self.Matrix(row,col));
